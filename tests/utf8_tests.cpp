...
TEST(UTF8, ValidAscii) {
    ...
    EXPECT_TRUE(s.valid());
    auto n = s.length(); ASSERT_TRUE(n.has_value());
    EXPECT_EQ(*n, 5u);
}
...
TEST(UTF8, ValidMultibyte) {
    ...
    auto n = s.length(); ASSERT_TRUE(n.has_value());
    EXPECT_EQ(*n, 8u);
    auto u32 = s.to_u32(); ASSERT_TRUE(u32.has_value());
    EXPECT_EQ((*u32)[0], U'H');
}

TEST(UTF8, RejectOverlong) {
    std::u8string overlong; overlong.push_back(static_cast<char8_t>(0xC0)); overlong.push_back(static_cast<char8_t>(0xAF));
...
TEST(UTF16BE, SurrogatePair) {
    ...
    auto n = be.length(); ASSERT_TRUE(n.has_value());
    EXPECT_EQ(*n, 1u);
    auto u32 = be.to_u32(); ASSERT_TRUE(u32.has_value());
    EXPECT_EQ((*u32)[0], U'🌍');
