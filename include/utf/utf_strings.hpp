...
struct encoding_traits<char8_t, E> {
    ...
    static inline bool decode_one(const unit_type* p, std::size_t n, char32_t& out, unsigned& consumed) noexcept {
        (void)order;
        if (n == 0) return false;
        ...
        if (need == 2) {
            ...
            out = static_cast<char32_t>(u); consumed = 2; return true;
        }
        ...
        if (need == 3) {
            ...
            out = static_cast<char32_t>(u); consumed = 3; return true;
        }
        ...
        out = static_cast<char32_t>(u); consumed = 4; return true;
    }
};
...
struct encoding_traits<char16_t, E> {
    ...
    static inline bool decode_one(const unit_type* p, std::size_t n, char32_t& out, unsigned& consumed) noexcept {
        ...
        unsigned u = 0x10000 + (((static_cast<unsigned>(w1) - 0xD800) << 10) | (static_cast<unsigned>(w2) - 0xDC00));
...
struct encoding_traits<char32_t, E> {
    ...
    static inline bool decode_one(const unit_type* p, std::size_t n, char32_t& out, unsigned& consumed) noexcept {
        ...
        out = static_cast<char32_t>(v); consumed = 1; return true;
    }
};

// ---------- Public API ----------
struct CodePointSpan { std::size_t unit_offset{}; std::size_t unit_length{}; };
struct DecodeError { std::size_t unit_offset{}; };
...
public:
    ...
    [[nodiscard]] bool valid() const noexcept {
        const auto* p = data_.data(); std::size_t n = data_.size();
        while (n) {
            char32_t cp{}; unsigned consumed{};
    ...
    [[nodiscard]] std::optional<std::size_t> length() const noexcept {
        const auto* p = data_.data(); std::size_t n = data_.size(); std::size_t count = 0;
        while (n) {
            char32_t cp{}; unsigned consumed{};
            if (!encoding_traits<Unit, E>::decode_one(p, n, cp, consumed)) return std::nullopt;
            ++count; p += consumed; n -= consumed;
        }
        return count;
    }

    [[nodiscard]] std::optional<std::u32string> to_u32() const {
        const auto* p = data_.data(); std::size_t n = data_.size(); std::u32string out; out.reserve(n);
        while (n) {
            char32_t cp{}; unsigned consumed{};
            if (!encoding_traits<Unit, E>::decode_one(p, n, cp, consumed)) return std::nullopt;
            out.push_back(cp); p += consumed; n -= consumed;
        }
        return out;
    }
    ...
    [[nodiscard]] std::optional<std::vector<CodePointSpan>> spans() const {
        const auto* p = data_.data(); std::size_t n = data_.size();
        std::vector<CodePointSpan> out; out.reserve(n);
        std::size_t off = 0;
        while (n) {
            char32_t cp{}; unsigned consumed{};
            if (!encoding_traits<Unit, E>::decode_one(p, n, cp, consumed)) return std::nullopt;
            out.push_back({off, consumed}); p += consumed; n -= consumed; off += consumed;
        }
        return out;
    }
...
[[nodiscard]] inline bool valid(const std::basic_string_view<Unit>& v) noexcept { return basic_utf_string<Unit, E>(v).valid(); }

template <class Unit, endian E>
[[nodiscard]] inline std::optional<std::size_t> length(const std::basic_string_view<Unit>& v) noexcept { return basic_utf_string<Unit, E>(v).length(); }

template <class Unit, endian E>
[[nodiscard]] inline std::optional<std::u32string> to_u32(const std::basic_string_view<Unit>& v) { return basic_utf_string<Unit, E>(v).to_u32(); }
